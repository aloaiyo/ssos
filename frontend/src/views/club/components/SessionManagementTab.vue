...
async function loadSeasons() {
  try {
    const response = await apiClient.get(`/clubs/${props.clubId}/seasons`)
...
async function saveSession() {
  ...
  try {
    ...
    if (editingSession.value) {
      ...
      await apiClient.post(`/clubs/${props.clubId}/sessions`, sessionData)
    }
    await loadSessions()
    closeCreateDialog()
  } catch (error) {
